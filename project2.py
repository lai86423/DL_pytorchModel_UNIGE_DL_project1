...
Original file is located at
    https://colab.research.google.com/drive/15dV2Vf3cRL1cJjERE1jWRWLeM-nf9NId
"""
...
def sigma(delta, weight, result):
    sig = torch.zeros(1, weight.size(0) - 1)
    # print('sig_size:', sig.size())
    for neu in range(weight.size(0) - 1):  # 神經元逐一計算相對的delta
        for x in range(weight.size(1)): # 利用weight的後一位判斷應該要勇的維度，判斷原先的輸出是否大於零
            if result[0][x] > 0:
